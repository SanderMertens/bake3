...
flecs::entity get_laser_beam(flecs::entity e) {
    flecs::entity head = e.target<prefabs::Laser::Head>();
    if (!head) {
        flecs::log::err("entity %s does not have prefabs::Laser::Head", e.str().c_str());
        return flecs::entity();
    }
    return head.target<prefabs::Laser::Head::Beam>();
}

flecs::entity get_barrel_left(flecs::entity e) {
    flecs::entity head = e.target<prefabs::Cannon::Head>();
    if (!head) {
        flecs::log::err("entity %s does not have prefabs::Cannon::Head", e.str().c_str());
        return flecs::entity();
    }
    return head.target<prefabs::Cannon::Head::BarrelLeft>();
}

flecs::entity get_barrel_right(flecs::entity e) {
    flecs::entity head = e.target<prefabs::Cannon::Head>();
    if (!head) {
        flecs::log::err("entity %s does not have prefabs::Cannon::Head", e.str().c_str());
        return flecs::entity();
    }
    return head.target<prefabs::Cannon::Head::BarrelRight>();
}
    ...
    Turret& turret, Target& target, Position& p) 
{
    ...
    if (enemy && enemy.is_alive()) {
        ...
        if (!e.has_second<prefabs::Laser>(flecs::IsA)) {
    ...
    Turret& turret, Target& target, Position& p)
{
    ...
    if (target.target && target.lock) {
        ...
        if (!is_laser) {
            ...
            flecs::entity barrel;
            if (turret.lr == -1) {
                barrel = get_barrel_left(e);
            } else {
                barrel = get_barrel_right(e);
            }
            turret.lr = -turret.lr;
...
{
    flecs::entity beam = get_laser_beam(it.entity(i));
...
void init_systems(flecs::world& ecs) {
    ...
    ecs.system<Position, Turret, Target>("BeamControl")
        .with(flecs::IsA, ecs.entity<prefabs::Laser>())
        .each(BeamControl);
