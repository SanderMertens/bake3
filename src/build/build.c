    ...
    bool emit_sources)
{
    ...
    char *marker_path = bake_join_path(deps_dir, bake_standalone_deps_marker);
    if (!marker_path || bake_write_file(marker_path, "generated by bake\n") != 0) {
...
static int bake_clean_project(const bake_project_cfg_t *cfg, bool recursive) {
    ...
    {
        char *deps_dir = bake_join_path(cfg->path, "deps");
        char *marker = deps_dir ? bake_join_path(deps_dir, bake_standalone_deps_marker) : NULL;
        if (!deps_dir || !marker) {
            ecs_os_free(deps_dir);
            ecs_os_free(marker);
            return -1;
